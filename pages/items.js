...
const showItems = (res) => {
  ...
  if (res.orderItems.length < 1) {
    ...
    domString = `<div style="display:grid;"><h1>ORDER TOTAL: $${res.orderItems.reduce((acc, curr) => acc + curr.price, 0)} </h1>`;
    res.orderItems.forEach((item) => {
      domString += `<div class="card" style="width: 18rem;">
      <div class="card-body">
        <h5 class="card-title">${item.itemName}</h5>
        <h6 class="card-subtitle mb-2 text-body-secondary">$${item.price}</h6>
        <a href="#" class="card-link" id="edit-item-btn--${item.firebaseKey}">Edit Item</a>
        <a href="#" class="card-link" id="delete-item-btn--${item.firebaseKey}">Delete Item</a>
      </div>
    </div>`;
    });
